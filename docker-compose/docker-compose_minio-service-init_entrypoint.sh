...
MQ_RETRIES=15;
# until curl --silent --insecure "http://mq-service:15672/api/queues" || [ "$MQ_RETRIES" -eq 0 ];
until curl --silent --insecure "http://$MQ_PORT:$MQ_PORT/api/users/rabbituser" || [ "$MQ_RETRIES" -eq 0 ];
do
  echo "[MinIO Init] Waiting MQ initialization...: $((MQ_RETRIES--))";
  sleep 5;
done;
